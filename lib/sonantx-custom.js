 ...
 *
 *  0 osc1_oct       10 osc2_vol       20 fx_delay_time
 *  1 osc1_det       11 osc2_waveform  21 fx_delay_amt
 ...
 *  6 osc2_oct       16 env_master     26 lfo_freq
 *  7 osc2_det       17 fx_filter      27 lfo_amt
...
const voice = (inst, n, L, R, at, rowLen, rate) => {
  ...
  // per sample here.
  const t1 =
    (pitch(n + (inst[0] - 8) * 12 + inst[1]) * (1 + 0.0008 * inst[2])) / k;
  const t2 =
    (pitch(n + (inst[6] - 8) * 12 + inst[7]) * (1 + 0.0008 * inst[8])) / k;
...
const renderSong = async (ctx, song, samples) => {
  ...
  for (const inst of song.songData) {
    ...
    for (let row = 0; row * rowLen < samples; row++) {
      ...
      const pat = inst[29][Math.floor(row / 32) % (song.endPattern + 1)] || 0;
      const n = pat ? (inst[30][pat - 1] || { n: [] }).n[row % 32] || 0 : 0;
