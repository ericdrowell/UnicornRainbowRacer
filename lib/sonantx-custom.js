...
const fold = (s) => {
  ...
  return Math.min(Math.max(y, -32768), 32767) / 32768;
};
 ...
 *
 * ── The instrument is an array, and these are its slots ────────────────────
 * **The SoundBox format names these fields and this does not.** A name like
 * `fx_resonance` is fifteen bytes wherever it appears, and it appears once per
 * instrument in every song plus once here for every field this reads — three
 * kilobytes of key across eight instruments, to say nothing that the position
 * in a fixed list does not say just as well.
 *
 * The songs on disk keep their names: `src/songs/*.json` is still SoundBox's
 * own shape, still what the bench opens and what a person edits. build.mjs
 * flattens each instrument on the way into the bundle, in exactly this order,
 * and that function is where the order is defined. Change it there and change
 * the numbers below with it — nothing checks, and a wrong index is not an error,
 * it is an instrument that sounds different.
 *
 *  0 osc1_offset    10 osc2_vol       20 fx_delay_time
 *  1 unused         11 osc2_waveform  21 fx_delay_amt
 *  2 osc1_detune    12 noise_fader    22 fx_pan_freq
 *  3 osc1_xenv      13 env_attack     23 fx_pan_amt
 *  4 osc1_vol       14 env_sustain    24 lfo_osc1_freq
 *  5 osc1_waveform  15 env_release    25 lfo_fx_freq
 *  6 osc2_offset    16 env_master     26 lfo_freq
 *  7 unused         17 fx_filter      27 lfo_amt
 *  8 osc2_detune    18 fx_freq        28 lfo_waveform
 *  9 osc2_xenv      19 fx_resonance   29 
 * 29 p   the pattern list       30 c   the patterns themselves
...
const voice = (inst, n, L, R, at, rowLen, rate) => {
  const osc1 = OSC[inst[5]];
  const osc2 = OSC[inst[11]];
  const lfo = OSC[inst[28]];
  ...
  const panFreq = 2 ** (inst[22] - 8) / rowLen;
  const lfoFreq = 2 ** (inst[26] - 8) / rowLen;
  ...
  const k = rate / 44100;
  const attack = inst[13] * k;
  const sustain = inst[14] * k;
  const release = inst[15] * k;
  const q = inst[19] / 255;
  // The build folds octave and semitone shifts into slots 0 and 6.
  ...
  // per sample here.
  const t1 =
    (pitch(n + inst[0]) * (1 + 0.0008 * inst[2])) / k;
  const t2 =
    (pitch(n + inst[6]) * (1 + 0.0008 * inst[8])) / k;
  const drive = 39 * inst[16];
  ...
  const swings = inst[24] || inst[25];
  const pans = inst[23];
  const mode = inst[17];
  const fixed = 1.5 * Math.sin((inst[18] * Math.PI) / rate);
  ...
  for (let j = 0; j < end && at + j < L.length; j++) {
    const swing = swings ? (lfo(j * lfoFreq) * inst[27]) / 512 + 0.5 : 0;
    ...
    if (inst[12]) s += (2 * Math.random() - 1) * inst[12] * e;
    s *= e / 255;
    ...
    if (mode) {
      const f = inst[25] ? 1.5 * Math.sin((inst[18] * swing * Math.PI) / rate) : fixed;
      low += f * band;
...
const echo = (inst, L, R, bpm, rate) => {
  const amt = inst[21] / 255;
  if (!amt) return;
  const d = Math.round(((inst[20] * 7.5) / bpm) * rate);
...
const renderSong = async (ctx, song, samples) => {
  ...
  // build.mjs rounds the exported row length to the player's integer tempo.
  const bpm = song.bpm;
  ...
  for (const inst of song.songData) {
    ...
    for (let row = 0; row * rowLen < samples; row++) {
      ...
      const pat = inst[29][Math.floor(row / 32) % (song.endPattern + 1)];
      const n = inst[30][pat - 1]?.[row % 32];
...
const renderNote = (ctx, inst, n, bpm = 120) => {
  const rate = ctx.sampleRate;
  const k = rate / 44100;
  const body = Math.ceil((inst[13] + inst[14] + inst[15]) * k);
  const d = Math.round(((inst[20] * 7.5) / bpm) * rate);
  const out = ctx.createBuffer(2, body + (inst[21] ? d * 6 : 0), rate);
  const L = out.getChannelData(0);
  const R = out.getChannelData(1);
